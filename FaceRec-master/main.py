...
def camera_recog():
    ...
    vs = cv2.VideoCapture("small2.mp4"); #"s3.mp4" #get input from webcam
    make_720p(vs)
    ...
    while True:
        ...
        if(len(aligns) > 0):
            ...
            for (i,rect) in enumerate(rects):
                ...
                cv2.putText(frame,facePos + ", "+ recog_data[i][0]+" - "+str(round(recog_data[i][1]))+"%",(rect[0],rect[1]),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),1,cv2.LINE_AA)
...
def findPeople(features_arr, positions, thres = 0.6, percent_thres = 70):
    ...
    for (i,features_128D) in enumerate(features_arr):
        ...
        if percentage <= percent_thres :
            result = "Student"
        returnRes.append((result+str(i),percentage))
    return returnRes    
